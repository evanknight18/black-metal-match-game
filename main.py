...
# Function to add a white background to tile1.png
def add_white_background(image_path, output_path):
    with Image.open(image_path) as img:
        ...
        # Save the result
        img_with_bg.save(output_path)

# Define the path to the images directory and the tile size
images_path = 'assets/images'
tile_size = (75, 75)

# Resize and crop all images in the images directory
for file_name in os.listdir(images_path):
    if file_name.endswith('.png'):
        image_path = os.path.join(images_path, file_name)
        resize_and_crop(image_path, image_path, tile_size)

# Add white background to tile1.png
add_white_background('assets/images/tile1.png', 'assets/images/tile1.png')

# Initialize Pygame
pygame.init()

# Screen settings
screen_width, screen_height = 600, 700  # Increased height for displaying score and moves
...
match4_sound = pygame.mixer.Sound('assets/sounds/match4.wav')  # New sound for matches of 4 or more
...
# Game variables
score = 0
moves = 30  # Total moves allowed
target_score = 1000  # Score to win the game
round_number = 1
game_over = False
    ...
    for row in range(len(board)):
        for col in range(len(board[row])):
            if board[row][col] is None:
                continue
            x = col * tile_size
            y = row * tile_size + 100  # Offset for scoreboard
...
def animate_swap(screen, board, tile_size, pos1, pos2):
    ...
    x1, y1 = pos1[0] * tile_size, pos1[1] * tile_size + 100  # Offset for scoreboard
    x2, y2 = pos2[0] * tile_size, pos2[1] * tile_size + 100  # Offset for scoreboard
...
def handle_matches(screen, board, tile_size):
    global score
...
def reset_game():
    global score, moves, target_score, board, game_over, round_number
    score = 0
    moves = 30
    target_score += 500
    round_number += 1
            ...
            if 0 <= col < 8 and 0 <= row < 8:  # Ensure click is within board bounds
                if selected_tile:
                    if is_adjacent(selected_tile, (col, row)):
                        ...
                        matches = handle_matches(screen, board, tile_size)
                        if not matches:
                            animate_swap(screen, board, tile_size, selected_tile, (col, row))  # Swap back if no match
                        while handle_matches(screen, board, tile_size):  # Handle combos
                            pass
                        selected_tile = None
